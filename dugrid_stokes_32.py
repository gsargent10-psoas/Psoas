...
class MicrogridGAN() :
	def __init__(self,mode=1,dataFolder='/app/data/',saveFolder='/app/results/',modelFolder='/app/results/',model='model_300.h5',epochs=300,comments='') :
		...
		if (self.mode==1) :
			...
			self.combined.compile(loss=['mse',HelperFunctions.mae_loss_stokes],loss_weights=[1,100],optimizer=optimizer,run_eagerly=True)
	...
	def load_training_data_stokes(self) :
		...
		for u,s in zip(ugrid,stokes):
			...
			for i in range(self.minibatchSize):
				while start_pix_x % 3 != 0 or start_pix_y % 2 != 0:
					start_pix_x = randint(1,2448-256)
					start_pix_y = randint(1,2048-256)
				ugrid_img = np.pad(self.imread(u),[[0,0],[0,self.pad]],'symmetric')[start_pix_y:start_pix_y+self.img_rows,start_pix_x:start_pix_x+self.img_cols]
				stokes_img = np.pad(self.imread(s),[[0,0],[0,self.pad],[0,0]],'symmetric')[start_pix_y:start_pix_y+self.img_rows,start_pix_x:start_pix_x+self.img_cols,:]
				s0_normalization = 2**17-1
				ugrid_normalization = 2**16-1

				ugrid_img_norm = tf.expand_dims(ugrid_img/ugrid_normalization,axis=-1)
				ugrid_img_norm = tf.concat([ugrid_img_norm,ugrid_img_norm,ugrid_img_norm],2)
				s0_img_norm = tf.expand_dims(stokes_img[:,:,0]/s0_normalization,axis=-1)
				s1_img_norm = tf.expand_dims(stokes_img[:,:,1]/stokes_img[:,:,0],axis=-1)
				s2_img_norm = tf.expand_dims(stokes_img[:,:,2]/stokes_img[:,:,0],axis=-1)

				input_img = tf.concat([input_img,tf.expand_dims(ugrid_img_norm,axis=0)],0)
			
				gt_img = tf.concat([gt_img,tf.expand_dims(tf.concat([s0_img_norm,s1_img_norm,s2_img_norm],2),axis=0)],0)
			
			print('input_img',input_img[1:,:,:,:].shape)
			print('gt_img',gt_img[1:,:,:,:].shape)
			
			yield input_img[1:,:,:,:],gt_img[1:,:,:,:],ugrid_name,stokes_name
	...
	def train(self):
		...
		for epoch in range(epochs):
			...
			d_avg = 0
			count = 0
			for batch_i, (input_img,gt_img,ugrid_name,stokes_name) in enumerate(self.load_training_data_stokes()):
			...
			if (epoch+1) % self.sample_interval == 0:
				stokes_image = gt_img[0,:,:,1].numpy()
				self.fs0_gt2.set_data(self.imgscale(stokes_image,stype="statistical"))
				self.fgt2.canvas.flush_events()
				stokes_image = gen_img[0,:,:,1]
				self.fs0_gen2.set_data(self.imgscale(stokes_image,stype="statistical"))
				self.fgen2.canvas.flush_events()
				self.fs0_gt.set_data(self.imgscale(gt_img[0,:,:,0].numpy(),stype="statistical"))
				self.fgt.canvas.flush_events()
				self.fs0_gen.set_data(self.imgscale(tf.clip_by_value(gen_img[0,:,:,0],0,2**17-1).numpy(),stype="statistical"))
				self.fgen.canvas.flush_events()
	...
	def test(self):
		...
		for batch_i, (full_image,gt_img,ugrid_name,stokes_name) in enumerate(self.load_test_data()):
			...
			row_iterations = (np.floor(full_image_rows/(sub_image_rows-(2*b+3))*1.5)).astype(np.int)
			col_iterations = (np.floor(full_image_cols/(sub_image_cols-(2*b+3))*1.5)).astype(np.int)
				...
				for c in range(col_iterations+1):
					...
					if ye >= full_image_rows:
						if xe >= full_image_cols:
							...
							stokes_image[ys:ye+1,xs:xe+1,2] = gen_image[0,:,:,2]
							breakoutflag=1
							break
						else:
							ye = full_image_rows-1
					...
					if (r==0):
						if (c==0):
							...
							gen_image[:,:,:,:] = self.generator.predict(sub_image)
							stokes_image[ys:ye+1,xs+b:xe+1,0] = gen_image[0,:,b::,0]
							stokes_image[ys:ye+1,xs+b:xe+1,1] = gen_image[0,:,b::,1]
							stokes_image[ys:ye+1,xs+b:xe+1,2] = gen_image[0,:,b::,2]
					elif (c==0):
						gen_image[:,:,:,:] = self.generator.predict(sub_image)
						stokes_image[ys+b:ye+1,xs:xe+1,0] = gen_image[0,b::,:,0]
						stokes_image[ys+b:ye+1,xs:xe+1,1] = gen_image[0,b::,:,1]
						stokes_image[ys+b:ye+1,xs:xe+1,2] = gen_image[0,b::,:,2]
					else:
						gen_image[:,:,:,:] = self.generator.predict(sub_image)
						stokes_image[ys+b:ye+1,xs+b:xe+1,0] = gen_image[0,b::,b::,0]
						stokes_image[ys+b:ye+1,xs+b:xe+1,1] = gen_image[0,b::,b::,1]
						stokes_image[ys+b:ye+1,xs+b:xe+1,2] = gen_image[0,b::,b::,2]
					system('clear')
					print('[Test Image %s]' % (ugrid_name))
					print('[Image Rows %s]' % (str(full_image_rows)))
					print('[Image Cols %s]' % (str(full_image_cols)))
